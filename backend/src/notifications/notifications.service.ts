// Notification backbone — the single dispatch path used by Phases 5/6, plus the read surface to list and
// mark notifications read (implementation plan Phase 4; architecture §9/§12/§16, CLAUDE §9). `notify` fans
// a notification out to a durable in-app inbox row, a live SSE push, and a queued email. Every write/read
// is tenant-scoped via `forTeam`; the read API is further scoped to the caller's own `userId` (no role
// gate, same posture as `standups /me/*`).
